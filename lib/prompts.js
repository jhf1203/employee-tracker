...
function modifyRoleEmplSel() {
  db.connection.query("SELECT id, first_name, last_name FROM employee", function(err, res) {
    ...
    inquirer.prompt([
      ...
      console.log(`changingempl before passing to next function:  ${changingEmpl}`)
...
function modifyMgrMgrSel(empl) {
  ...
  db.connection.query("SELECT id, first_name, last_name FROM employee WHERE is_manager = 1", function (err, res) {
    inquirer.prompt([
      ...
      let query = db.connection.query("UPDATE employee SET manager_id = ? WHERE id = ?", [mgr, changingEmpl], function(err, res){
        if (err) {
        } else {
          console.log("Changed!")
          continueOption();
