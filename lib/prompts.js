...
function modifyMgrEmplSel() {
  db.connection.query("SELECT id, first_name, last_name FROM employee", function(err, res) {
    ...
    inquirer.prompt([
      ...
      console.log("the employee we're passing into the vara is ", empl.modifyMgrChangedE)
      const changingEmpl = parseInt(empl.modifyMgrChangedE.slice(0,5));
      console.log(`before the new function the cmployee is ${changingEmpl}`)
...
function modifyMgrMgrSel(empl) {
  ...
  db.connection.query("SELECT id, first_name, last_name FROM employee WHERE is_manager = 1", function (err, res) {
    inquirer.prompt([
      ...
      console.log(`this is the mgr var ${mgr}, and it was ${people.modifyMgrChangedM} before I defined it as mgr.`)
      const changingEmpl = people.employee
      console.log(`this is the empl var ${changingEmpl}`)
      let query = db.connection.query("UPDATE employee SET manager_id = ? WHERE id = ?", [mgr, empl], function(err, res){
